...
/**
 * This class is used to present context information to the user. If multiple
 * contexts are valid at the current cursor location, a list is presented from
 * which the user may choose one context. Once the user makes their choice, or
 * if there was only a single possible context, the context information is shown
 * in a tool tip like popup.
 * <p>
 * If the tool tip is visible and the user wants to see context information of a
 * context embedded into the one for which context information is displayed,
 * context information for the embedded context is shown. As soon as the cursor
 * leaves the embedded context area, the context information for the embedding
 * context is shown again.
 * 
...
class ContextInformationPopup implements IContentAssistListener {
	...
	static class ContextFrame {
		 ...
		 * @since 3.1
		 */
		public ContextFrame(IContextInformation information, int beginOffset,
				int offset, int visibleOffset,
				IContextInformationValidator validator,
				IContextInformationPresenter presenter) {
		...
		public boolean equals(Object obj) {
			if (obj instanceof ContextFrame) {
				...
				return fInformation.equals(frame.fInformation)
						&& fBeginOffset == frame.fBeginOffset;
			}
	...
	/**
	 * Selection listener on the text widget which is active while a context
	 * information pop up is shown.
	 * 
	 * @since 3.0
	 */
	 ...
	 *            the content assist subject control on top of which the context
	 ...
	 * @return a potential error message or <code>null</code> in case of no
	 *         error
	 */
		...
		BusyIndicator.showWhile(control.getDisplay(), new Runnable() {
			public void run() {
				...
				if (count == 1) {
					...
					// if any of the proposed context matches any of the
					// contexts on the stack,
					// assume that one (so, if context info is invoked
					// repeatedly, the current
					// info is kept)
					int index = 0;
					for (int i = 0; i < contexts.length; i++) {
						...
						for (Iterator it = fContextFrameStack.iterator(); it
								.hasNext();) {
					...
					if (fLineDelimiter == null)
						fLineDelimiter = fContentAssistSubjectControlAdapter
								.getLineDelimiter();
	 ...
	 * @since 3.0
	 */
	private ContextFrame createContextFrame(IContextInformation information,
			int offset) {
		IContextInformationValidator validator = fContentAssistSubjectControlAdapter
				.getContextInformationValidator(fContentAssistant, offset);
		...
		if (validator != null) {
			...
			if (beginOffset == -1)
				beginOffset = offset;
			int visibleOffset = fContentAssistSubjectControlAdapter
					.getWidgetSelectionRange().x - (offset - beginOffset);
			...
			return new ContextFrame(information, beginOffset, offset,
					visibleOffset, validator, presenter);
		}
	...
	/**
	 * Compares <code>frame</code> with the top of the stack, returns
	 * <code>true</code> if the frames are the same.
	 * 
	 * @param frame
	 *            the frame to check
	 * @return <code>true</code> if <code>frame</code> matches the top of the
	 *         stack
	 * @since 3.0
	 */
	...
	/**
	 * Compares <code>frame</code> with most recently removed context frame,
	 * returns <code>true</code> if the frames are the same.
	 * 
	 * @param frame
	 *            the frame to check
	 * @return <code>true</code> if <code>frame</code> matches the most recently
	 *         removed
	 * @since 3.0
	 */
	...
	private void internalShowContextFrame(ContextFrame frame, boolean initial) {
		...
		if (frame.fPresenter != null) {
			...
			fContentAssistSubjectControlAdapter
					.installContextInformationPresenter(frame);
			frame.fPresenter.updatePresentation(frame.fOffset,
					fTextPresentation);
		}

		createContextInfoPopup();

		fContextInfoText.setText(frame.fInformation
				.getInformationDisplayString());
		if (fTextPresentation != null)
			TextPresentation.applyTextPresentation(fTextPresentation,
					fContextInfoText);
		resize(frame.fVisibleOffset);

		if (initial) {
			if (fContentAssistant.addContentAssistListener(this,
					ContentAssistant.CONTEXT_INFO_POPUP)) {
				if (fContentAssistSubjectControlAdapter.getControl() != null) {
					...
					fContentAssistSubjectControlAdapter
							.addSelectionListener(fTextWidgetSelectionListener);
				}
				fContentAssistant
						.addToLayout(
								this,
								fContextInfoPopup,
								ContentAssistant.LayoutManager.LAYOUT_CONTEXT_INFO_POPUP,
								frame.fVisibleOffset);
				fContextInfoPopup.setVisible(true);
			}
		} else {
			fContentAssistant.layout(
					ContentAssistant.LayoutManager.LAYOUT_CONTEXT_INFO_POPUP,
					frame.fVisibleOffset);
		}
	}
	...
	private IContextInformation[] computeContextInformation(int offset) {
		return fContentAssistSubjectControlAdapter.computeContextInformation(
	...
	/**
	 * Creates the context information popup. This is the tool tip like overlay
	 * window.
	 */
	private void createContextInfoPopup() {
		...
		fContextInfoPopup = new Shell(control.getShell(), SWT.NO_TRIM
				| SWT.ON_TOP);
		fContextInfoPopup
				.setBackground(display.getSystemColor(SWT.COLOR_BLACK));

		fContextInfoText = new StyledText(fContextInfoPopup, SWT.MULTI
	...
	private void resize(int offset) {
		Point size = fContextInfoText.computeSize(SWT.DEFAULT, SWT.DEFAULT,
				true);
		final int TEXT_PAD = 0;
		final int BORDER_PAD = 2;
		final int PAD = TEXT_PAD + BORDER_PAD;
		size.x += PAD;
		Rectangle bounds = fContentAssistant.getLayoutManager()
				.computeBoundsAboveBelow(fContextInfoPopup, size, offset);
		if (bounds.width < size.x)
			// we don't fit on the screen - try again and wrap
			size = fContextInfoText.computeSize(bounds.width - PAD,
					SWT.DEFAULT, true);
	...
	/**
	 * Creates the context selector in case the user has the choice between
	 * multiple valid contexts at a given offset.
	 */
	private void createContextSelector() {
		...
		Control control = fContentAssistSubjectControlAdapter.getControl();
		fContextSelectorShell = new Shell(control.getShell(), SWT.ON_TOP
				| SWT.RESIZE);
		GridLayout layout = new GridLayout();
		layout.marginWidth = 0;
		layout.marginHeight = 0;
		fContextSelectorShell.setLayout(layout);
		fContextSelectorShell.setBackground(control.getDisplay()
		...
		fContentAssistant.addToLayout(this, fContextSelectorShell,
				ContentAssistant.LayoutManager.LAYOUT_CONTEXT_SELECTOR,
				fContentAssistant.getSelectionOffset());
	}

	/**
	 * Returns the minimal required height for the popup, may return 0 if the
	...
	int getMinimalHeight() {
		...
		if (Helper.okToUse(fContextSelectorTable)) {
			...
			Rectangle trim = fContextSelectorTable.computeTrim(0, 0,
					SWT.DEFAULT, items);
			height = trim.height;
		}
		return height;
	...
	/**
	 * Causes the context information of the context selected in the context
	 * selector to be displayed in the context information popup.
	 */
	private void insertSelectedContext() {
		...
		internalShowContextInfo(createContextFrame(fContextSelectorInput[i],
				offset));
	}
	 ...
	 *            the index of the proposal to select
	 */
	...
	private void displayContextSelector() {
		if (fContentAssistant.addContentAssistListener(this,
				ContentAssistant.CONTEXT_SELECTOR))
	...
	public boolean hasFocus() {
		if (Helper.okToUse(fContextSelectorShell))
			return (fContextSelectorShell.isFocusControl() || fContextSelectorTable
					.isFocusControl());
	...
	/**
	 * Returns whether this context information popup is active. I.e., either a
	 * context selector or context information is displayed.
	 * 
	 * @return <code>true</code> if the context selector is active
	 */
	public boolean isActive() {
		return (Helper.okToUse(fContextInfoPopup) || Helper
				.okToUse(fContextSelectorShell));
	}
	...
	private boolean contextSelectorKeyPressed(VerifyEvent e) {
		...
		if (key == 0) {
			...
			int visibleRows = (fContextSelectorTable.getSize().y / fContextSelectorTable
					.getItemHeight()) - 1;
	...
	/**
	 * Validates the context information for the viewer's actual cursor
	 * position.
	 */
	private void validateContextInformation() {
		/*
		 * Post the code in the event queue in order to ensure that the action
		 * described by this verify key event has already been executed.
		...
		fContextInfoPopup.getDisplay().asyncExec(new Runnable() {
			...
			private ContextFrame fFrame = (ContextFrame) fContextFrameStack
					.peek();

			public void run() {
				// only do this if no other frames have been added in between
				if (!fContextFrameStack.isEmpty()
						&& fFrame == fContextFrameStack.peek()) {
					int offset = fContentAssistSubjectControlAdapter
							.getSelectedRange().x;

					// iterate all contexts on the stack
					while (Helper.okToUse(fContextInfoPopup)
							&& !fContextFrameStack.isEmpty()) {
						ContextFrame top = (ContextFrame) fContextFrameStack
								.peek();
						if (top.fValidator == null
								|| !top.fValidator
										.isContextInformationValid(offset)) {
							hideContextInfoPopup(); // loop variant: reduces the
													// number of contexts on the
													// stack
						} else if (top.fPresenter != null
								&& top.fPresenter.updatePresentation(offset,
										fTextPresentation)) {
							int widgetOffset = fContentAssistSubjectControlAdapter
									.getWidgetSelectionRange().x;
							TextPresentation.applyTextPresentation(
									fTextPresentation, fContextInfoText);
							resize(widgetOffset);
							break;
						} else
							break;
